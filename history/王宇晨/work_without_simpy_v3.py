...
"""
    author: wang heyu
    corrected by :wang yuchen
    
    To-do:
        1. Test and debug(after correcting)
        2. Format printing
        3. Multiple files
        4. Comments
"""
"""
ptr: pointer 指针（指向丢包点）
src: source 源对象
"""
...
class Acknowledge_character:
    def __init__(self,ack_num,src):
...
class Sender:
    ...
    def awake(self):
        ...
        ack = Acknowledge_character(self.cnt,self)
        self.receiver.get_ack(ack)
...
class Receiver:
    ...
    def put(self, packet, resend=False):
        ...
        """if resend == False:
            self.data_cnt[p_src] += 1"""#(delete)

    def get_ack(self,ack_num):
        g_ack = ack_num.ack_num
        g_src = ack_num.src.id
        self.data_cnt[g_src] = g_ack
